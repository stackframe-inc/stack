...
<AccordionGroup>
  <Accordion title="What languages are supported?">
    In the frontend, Stack supports TypeScript and JavaScript with Next.js, React, and vanilla JavaScript. In the backend, Stack has a flexible [REST API](/rest-api) that can be used with any language or framework.
  </Accordion>
  <Accordion title="Can I use Stack with other JavaScript frameworks, like Astro or Angular?">
    While you can use any backend framework with Stack, the frontend officially supports Next.js, React, and vanilla JavaScript. If you want to use a different frontend framework, you will have to build the integration yourself with the client endpoints of our [REST API](/rest-api). Some members of our community have started projects to do this, so you may want to join [our Discord](https://discord.stack-auth.com) to coordinate with them.
  </Accordion>
